...
function changeBtnClass(index,className){
  button.forEach(function(elem){
    elem.classList.remove(className);
  })
  button[index].classList.add(className);
}
...
tiles.forEach(function(element,index){
  tiles[index].addEventListener("click",function(){
    ...
    console.log("Clicked:" + this.textContent + " To click:" + count );
  }
    if (count==41){
    for(var i = 0 ; i <20 ; i++){
...
function animeSquares(){
 ...
 for(var i=0 ; i<2 ;i++){
   for(var j=1 ; j<11 ; j++){
   tiles[i*10+ j-1 ].classList.add("square" + j );
    }
}
 generateArray(nos);
 for ( var i = 0 ; i < nos ; i++){
...
function noAnimeSquare(){
  box.forEach(function(element,index){
   ...
   for(var i=0 ; i<2 ;i++){
     for(var j=1 ; j<=10 ; j++){
     tiles[i*10+ j-1].classList.remove("square"+ j);
    }
  }
}
...
function init(){ 
  ...
  timerStarted = true ;
  var t1 = setTimeout(function(){ noleaderboard(); newGame.classList.remove("on"); countDown.textContent = "3";  },1000);
  var t2 = setTimeout(function(){ clearTimeout(t1); countDown.textContent = "2";  },2000);
  var t3 = setTimeout(function(){ clearTimeout(t2); countDown.textContent = "1";  },3000);
  var t4 = setTimeout(function(){ timerStarted = false; clearTimeout(t3); countDown.style.display="none"; 
  timeIt.start();
